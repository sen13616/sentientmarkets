export default function Loading() {
  // Light skeleton — the routes behind it are the white `home-light` pages,
  // so a dark skeleton here caused a dark flash before content painted.
  return (
    <div style={{ background: '#ffffff', minHeight: '100vh' }}>
      <div style={{
        ...
        <style>{`
          ...
          .skel { background: rgba(0,0,0,0.06); border-radius: 6px; animation: pulse 1.8s ease-in-out infinite; }
        `}</style>

        {/* Nav skeleton */}
        <div style={{ height: 54, borderBottom: '1px solid rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', padding: '0 32px', gap: 16 }}>
        ...
        {/* Hero skeleton */}
        <div style={{ padding: '80px 32px 72px', borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
        ...
        {/* F&G skeleton */}
        <div style={{ padding: '40px 32px', borderBottom: '1px solid rgba(0,0,0,0.08)', display: 'grid', gridTemplateColumns: '300px 1fr', gap: 0 }}>
        ...
        {/* Indices skeleton */}
        <div style={{ padding: '40px 32px', borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
          <div className="skel" style={{ width: 120, height: 10, marginBottom: 20, borderRadius: 4 }} />
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 10, overflow: 'hidden' }}>
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} style={{ padding: '22px 24px', borderRight: i < 3 ? '1px solid rgba(0,0,0,0.08)' : 'none', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div style={{ padding: '40px 32px', borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
          ...
          <div style={{ border: '1px solid rgba(0,0,0,0.08)', borderRadius: 10, overflow: 'hidden' }}>
            <div style={{ padding: '10px 20px', background: '#f7f7f7', borderBottom: '1px solid rgba(0,0,0,0.08)', display: 'grid', gridTemplateColumns: '40px 80px 1fr 100px 110px', gap: 16 }}>
              {[30, 40, 60, 60, 70].map((w, i) => (
                <div key={i} className="skel" style={{ height: 8, width: w }} />
              ))}
            </div>
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} style={{ padding: '13px 20px', borderBottom: '1px solid rgba(0,0,0,0.05)', display: 'grid', gridTemplateColumns: '40px 80px 1fr 100px 110px', gap: 16, alignItems: 'center' }}>
