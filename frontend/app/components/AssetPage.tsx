...
export default async function AssetPage({ ticker: tickerParam, assetTypeHint }: AssetPageProps) {
  ...
  // Asset type flags
  const assetType         = (data as any).asset_type ?? 'stock';
  const assetMeta         = (data as any).asset_meta ?? {};
  ...
  const hasFundamentals   = assetMeta.has_fundamentals   ?? isStock;
  const hasAnalyst        = assetMeta.has_analyst        ?? (isStock || isETF);
  const hasInstitutional  = assetMeta.has_institutional  ?? (isStock || isETF);
  const hasInsider        = assetMeta.has_insider        ?? isStock;
  const hasIndexComparison = assetMeta.has_index_comparison ?? (isStock || isETF);
  const hasFearGreed      = assetMeta.has_fear_greed     ?? (isStock || isETF || isIndex);
  const hasReddit         = assetMeta.has_reddit         ?? (isStock || isETF || isCrypto);
  ...
  return (
    <div className={styles.page}>
          ...
          <div className={styles.stockTopRow}>
            <span className={styles.stockTicker}>
              {isIndex
                ...
                : (isForex || isCrypto)
                  ? (assetMeta.display_name || ticker)
                  : ticker}
            </span>
            {isIndex
              ? <span className={styles.stockName}>{ticker}</span>
              : (!isForex && !isCrypto)
            ...
            <div className={styles.stockBadges}>
              {isForex ? (
                <>
                  <span className={styles.stockBadge}>Forex</span>
                  {assetMeta.base_currency && <span className={styles.stockBadge}>{assetMeta.base_currency}</span>}
                  {assetMeta.quote_currency && <span className={styles.stockBadge}>{assetMeta.quote_currency}</span>}
                </>
              ) : isCrypto ? (
                <span className={styles.stockBadge}>Crypto</span>
              ) : isIndex ? (
                <span className={styles.stockBadge}>Index</span>
              ) : isCommodity ? (
                <span className={styles.stockBadge}>Commodity</span>
              ) : (
                <>
                  {(data as any).exchange && <span className={styles.stockBadge}>{(data as any).exchange}</span>}
                  {fund.sector && <span className={styles.stockBadge}>{fund.sector}</span>}
                </>
              )}
            </div>
          </div>
          <div className={styles.stockPriceRow}>
            ...
            <div className={styles.stockChangeBlock}>
              <div className={styles.stockChange}>
                ...
                <span className={(price.change ?? 0) >= 0 ? styles.pos : styles.neg}>
                  {isForex ? '' : '$'}{Math.abs(price.change ?? 0).toFixed(isForex ? (assetMeta.price_decimals ?? 4) : 2)}
                </span>
                <span className={(price.change ?? 0) >= 0 ? styles.pos : styles.neg}>
              ...
              {!isForex && !isCrypto && price.post_market_price != null && (
          ...
          <div className={styles.chartPane}>
            {priceHistory.dates?.length > 0
              ...
              <PriceChart
                ...
                showIndexComparison={hasIndexComparison}
              />
            ) : (
            ...
            return (
              <div className={styles.pillar}>
                ...
                <div className={styles.pillarBody}>
                  ...
                  {(hasInsider || hasReddit) && <div className={styles.pCell}>
            ...
            return (
              <div className={styles.pillar}>
                ...
                <div className={styles.pillarBody}>
                  ...
                  {/* Cell 1: F&G */}
                  {hasFearGreed && <div className={styles.pCell}>
      ...
      {/* ── 6. FUNDAMENTALS ────────────────────────────────────── */}
      {hasFundamentals && <FadeIn delay={60}>
      <div className={styles.section}>
      ...
      {/* ── 8. INSTITUTIONAL ───────────────────────────────────── */}
      {hasInstitutional && <FadeIn delay={60}>
      <div className={styles.section}>
